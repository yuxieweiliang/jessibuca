...
async function WebRTCVideo(player) {
    ...
    function ontrack (event, __webRTC) {
        clearTimeout(__timer)
        if (event.track.kind === "video") {
            if (isReset) {
                __timer = setTimeout(function() {
                    console.log(webRTC, __webRTC)
    ...
    function resetWebRTC () {
        const __webRTC = new WebRtcPeerRecvOnly();
        clearTimeout(__TimerWebRTC)

        __webRTC.pc.ontrack = function(event) {
            ontrack(event, __webRTC)
        }

        // restartVideo(event.streams[0])
        // $video = null;
        fetchRemoteSDP (__webRTC)

        console.log('restart WebRTC', webRTC)
    ...
    webRTC.pc.ontrack = function(event) {
